...
{
    char code;
    int num_parts = 0, max_parts = INITIAL_PARTS;
    ...
    for (;;) {
        ...
        while (getchar() != '\n')   /* skips to end of line */
            ;
        switch (code) {
            case 'i': insert(&inventory, &num_parts, &max_parts); break;
...
void insert(struct part **inventory, int *num_parts, int *max_parts)
{
    ...
    if (*num_parts == *max_parts) {
        *max_parts += ADDITIONAL_PARTS;
        struct part *temp = realloc(*inventory, *max_parts * sizeof(*temp));
        if (temp != NULL)
            *inventory = temp;
        else
            printf("Error: Memory heap exhausted. Database full.\n");
    }
