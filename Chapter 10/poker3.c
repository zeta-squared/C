/* Classifies a poker hand */

#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>

#define NUM_CARDS 5
#define RANK 0
#define SUIT 1
#define ACE_HIGH_RANK 13

/* external variables */
bool royal_flush, straight, ace_low_straight, flush, four, three;
int pairs;  /* can be 0, 1 or 2 */

/* prototypes */
void read_cards(int hand[][2], int ace_low_hand[][2]);
void analyse_hand(int hand[][2], int ace_low_hand[][2]);
void print_result(void);

/******************************************************************************
 * main: Calls read_cards, analyse_hand and print_result repeatedly.          *
 ******************************************************************************/
int main(void)
{
    int hand[NUM_CARDS][2] = {0}, ace_low_hand[NUM_CARDS][2] = {0};

    for (;;) {
        read_cards(hand, ace_low_hand);
        analyse_hand(hand, ace_low_hand);
        print_result();
    }
}

/******************************************************************************
 * read_cards: Reads the cards into external variables; checks for bad cards  *
 *             and duplicate cards.                                           *
 ******************************************************************************/
void read_cards(int hand[][2], int ace_low_hand[][2])
{
    char ch, rank_ch, suit_ch;
    int rank, suit, i;
    bool bad_card, duplicate;
    int cards_read = 0;

    while (cards_read < NUM_CARDS) {
        bad_card = false;
        duplicate = false;

        printf("Enter a card: ");

        rank_ch = getchar();
        switch (rank_ch) {
            case '0':           exit(EXIT_SUCCESS);
            case '2':           rank = 1; break;
            case '3':           rank = 2; break;
            case '4':           rank = 3; break;
            case '5':           rank = 4; break;
            case '6':           rank = 5; break;
            case '7':           rank = 6; break;
            case '8':           rank = 7; break;
            case '9':           rank = 8; break;
            case 't': case 'T': rank = 9; break;
            case 'j': case 'J': rank = 10; break;
            case 'q': case 'Q': rank = 11; break;
            case 'k': case 'K': rank = 12; break;
            case 'a': case 'A': rank = 13; break;
            default:            bad_card = true;
        }

        suit_ch = getchar();
        switch (suit_ch) {
            case 'c': case 'C': suit = 0; break;
            case 'd': case 'D': suit = 1; break;
            case 'h': case 'H': suit = 2; break;
            case 's': case 'S': suit = 3; break;
            default:            bad_card = true;
        }

        while ((ch = getchar()) != '\n')
            if (ch != ' ') bad_card = true;

        for (i = 0; i < cards_read; i++) {
            if (hand[i][RANK] == rank && hand[i][SUIT] == suit) {
                duplicate = true;
                break;
            }
        }

        if (bad_card)
            printf("Bad card; ignored.\n");
        else if (duplicate) {
            printf("Duplicate card; ignored.\n");
        }
        else if (cards_read == 0) {
            hand[cards_read][RANK] = rank;
            hand[cards_read][SUIT] = suit;
            cards_read++;
        } else {
            for (i = cards_read - 1; i >= 0; i--) {
                if (i == 0 && rank < hand[i][RANK]) {
                    hand[i+1][RANK] = hand[i][RANK];
                    hand[i+1][SUIT] = hand[i][SUIT];
                    hand[i][RANK] = rank;
                    hand[i][SUIT] = suit;
                } else if (rank < hand[i][RANK]) {
                    hand[i+1][RANK] = hand[i][RANK];
                    hand[i+1][SUIT] = hand[i][SUIT];
                } else {
                    hand[i+1][RANK] = rank;
                    hand[i+1][SUIT] = suit;
                    break;
                }
                
            }
            cards_read++;
        }
    }

    if (hand[NUM_CARDS-1][RANK] == ACE_HIGH_RANK) {
        ace_low_hand[0][SUIT] = hand[NUM_CARDS-1][SUIT];
        for (i = 0; i < NUM_CARDS-1; i++) {
            ace_low_hand[i+1][RANK] = hand[i][RANK];
            ace_low_hand[i+1][SUIT] = hand[i][SUIT];
        }
    }
}

/******************************************************************************
 * analyse_hand: Determines whether the hand contains a straight, a flush,    *
 *               four-of-a-kind, and/or three-of-a-kind; determines the       *
 *               number of apirs; stores the results into external variables. *
 ******************************************************************************/
void analyse_hand(int hand[][2], int ace_low_hand[][2])
{
    int num_consec = 1;
    int i, j, suit;

    royal_flush = false;
    straight = false;
    ace_low_straight = false;
    flush = false;
    four = false;
    three = false;
    pairs = 0;

    /* check for royal flush */
    if (hand[0][RANK] == 8)
        for (i = 1; i < NUM_CARDS; i++)
            if ((hand[i][RANK] - 1) == hand[i-1][RANK] && hand[i][SUIT] == hand[0][SUIT])
                num_consec++;

    if (num_consec == NUM_CARDS) {
        royal_flush = true;
        return;
    }

    /* check for flush */
    for (i = 1; i < NUM_CARDS; i++) {
        if (hand[i][SUIT] == hand[0][SUIT] && i == NUM_CARDS)
            flush = true;
        else
            break;
    }

    /* check for straight */
    num_consec = 1;
    for (i = 1; i < NUM_CARDS; i++)
        if ((hand[i][RANK] - 1) == hand[i-1][RANK])
            num_consec++;

    if (num_consec == NUM_CARDS) {
        straight = true;
        return;
    }

    /* check for ace low straight */
    num_consec = 1;
    for (i = 1; i < NUM_CARDS; i++) {
        if ((ace_low_hand[i][RANK] - 1) == ace_low_hand[i-1][RANK])
            num_consec++;
    }

    if (num_consec == NUM_CARDS) {
        ace_low_straight = true;
        return;
    }

    /* check for 4-of-a-kind, 3-of-a-kind and pairs */
    for (i = 0; i < NUM_CARDS; i++) {
        num_consec = 1;
        for (j = i + 1; j < NUM_CARDS; j++) {
            if (hand[i][RANK] == hand[j][RANK]) {
                num_consec++;
            }
        }
        if (num_consec == 4) four = true;
        if (num_consec >= 3) three = true;
        if (num_consec >= 2) pairs++;
    }
}

/******************************************************************************
 * print_result: Notifies the user of the result, using the external          *
 *               variables set by analyse_hand.                               *
 ******************************************************************************/
void print_result(void)
{
    if (royal_flush)
        printf("Royal flush");
    else if (straight && flush)
        printf("Straight flush");
    else if (four)
        printf("Four of a kind");
    else if (three && pairs == 1)
        printf("Full house");
    else if (flush)
        printf("Flush");
    else if (straight)
        printf("Straight");
    else if (ace_low_straight)
        printf("Ace low straight");
    else if (three)
        printf("Three of a kind");
    else if (pairs == 2)
        printf("Two pairs");
    else if (pairs == 1)
        printf("Pair");
    else
        printf("High card");

    printf("\n\n");
}
